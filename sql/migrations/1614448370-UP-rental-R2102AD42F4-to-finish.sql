-- 1614448370 UP rental-R2102AD42F4-to-finish

-- RENTA R2102AD42F4

-- UPDATE RENTAL

UPDATE rental set   kilometers_init = 100 , kilometers_end = 110 , rent_status = 3 , penalty_days = 0 , received_at = "2021-02-21 20:36:00" , received_by = (select id from users where email = "igalaviz@allabordo.com") , delivered_at = "2021-02-21 20:37:00" , delivered_by =  (select id from users where email = "igalaviz@allabordo.com")  where reservation_code = "R2102AD42F4" ;


UPDATE rental set payment_status = '1' where reservation_code = "R2102AD42F4" ;
...
, (select id from rental where reservation_code = "R2102AD42F4")
, 148.69
, 6078
, 6078
, 0.00
...
insert into tickets_details(amount,quantity,detail,ticket_id,unit_price,created_by)
values (1078.0 , 1 , 'Pago parcial renta' , (SELECT @idTicket) , 1078.0 ,  ( select id from users where email = "igalaviz@allabordo.com" ) );
...
values ( 5000 , true , 1 , 22 , false , ( select id from users where email = "igalaviz@allabordo.com" ) , (select id from rental where reservation_code = "R2102AD42F4") , (SELECT @idTicket) ,'cash' );
SET @idPaymentGarantia = (SELECT last_insert_id());

insert into payment(amount,is_deposit,payment_method_id,currency_id,is_extra_charge,created_by,rental_id,ticket_id,payment_method)
values ( 1078 , false , 1 , 22 , false , ( select id from users where email = "igalaviz@allabordo.com" ) , (select id from rental where reservation_code = "R2102AD42F4") , (SELECT @idTicket) ,'cash' );
...
VALUES (
(select id from cash_out where employee_id = ( select id from employee where user_id = (select id from users where email = "igalaviz@allabordo.com")) and cash_out_status = 1)
, (SELECT @idPaymentAnticipo ), 1078.0 , '0' , ( select id from users where email = "igalaviz@allabordo.com" ) );
...
values ( (select id from rental where reservation_code = "R2102AD42F4") , 10 , 6 , 0 , 0 , 6 , 0 , 0.0 , 0.0 , 1 , 1 , ( select id from users where email = "igalaviz@allabordo.com" ) );
