-- 1615060406 UP status-rental-R21018BA8D3-to-transit


-- UPDATE RENTAL R21018BA8D3

UPDATE rental set payment_status = '1',   kilometers_init = 100 ,  rent_status = 2  , delivered_at = '2021-03-01 07:05:00' , delivered_by =  (select id from users where email = "igalaviz@allabordo.com")  where reservation_code = "R21018BA8D3";
...
, (select id from rental where reservation_code = "R21018BA8D3")
, 894.90
, 11488
, 11488
, 0.00
...
insert into tickets_details(amount,quantity,detail,ticket_id,unit_price,created_by)
values (6488.0 , 1 , 'Pago parcial renta' , (SELECT @idTicket) , 6488.0 ,  ( select id from users where email = "igalaviz@allabordo.com" ) );
...
values ( 5000 , true , 1 , 22 , false , ( select id from users where email = "igalaviz@allabordo.com" ) , (select id from rental where reservation_code = "R21018BA8D3") , (SELECT @idTicket) ,'cash' );
SET @idPaymentGarantia = (SELECT last_insert_id());

insert into payment(amount,is_deposit,payment_method_id,currency_id,is_extra_charge,created_by,rental_id,ticket_id,payment_method)
values ( 6488 , false , 1 , 22 , false , ( select id from users where email = "igalaviz@allabordo.com" ) , (select id from rental where reservation_code = "R21018BA8D3") , (SELECT @idTicket) ,'cash' );
...
VALUES (
(select id from cash_out where employee_id = ( select id from employee where user_id = (select id from users where email = "igalaviz@allabordo.com")) and cash_out_status = 1)
, (SELECT @idPaymentAnticipo ), 6488.0 , '0' , ( select id from users where email = "igalaviz@allabordo.com" ) );
...
values ( (select id from rental where reservation_code = "R21018BA8D3") , 10 , 6 , 0 , 0 , 6 , 0 , 0.0 , 0.0 , 0 , 1 , ( select id from users where email = "igalaviz@allabordo.com" ) );
