-- 1614125892 UP rental-R21023288B6-to-finished


-- UPDATE RENTAL

UPDATE rental set payment_status = 1,  kilometers_init = 100 , kilometers_end = 110 , rent_status = 3 , penalty_days = 0 , received_at = "2021-02-23 17:36:00" , received_by = (select id from users where email = "igalaviz@allabordo.com")  where reservation_code = "R21023288B6" ;
...
, (select id from rental where reservation_code = "R21023288B6")
, 289.10
, 7096
, 7096
, 0.00
...
insert into tickets_details(amount,quantity,detail,ticket_id,unit_price,created_by)
values (2096.0 , 1 , 'Pago parcial renta' , (SELECT @idTicket) , 2096.0 ,  ( select id from users where email = "igalaviz@allabordo.com" ) );
...
values ( 5000 , true , 1 , 22 , false , ( select id from users where email = "igalaviz@allabordo.com" ) , (select id from rental where reservation_code = "R21023288B6") , (SELECT @idTicket) ,'cash' );
SET @idPaymentGarantia = (SELECT last_insert_id());

insert into payment(amount,is_deposit,payment_method_id,currency_id,is_extra_charge,created_by,rental_id,ticket_id,payment_method)
values ( 2096 , false , 1 , 22 , false , ( select id from users where email = "igalaviz@allabordo.com" ) , (select id from rental where reservation_code = "R21023288B6") , (SELECT @idTicket) ,'cash' );
...
VALUES (
(select id from cash_out where employee_id = ( select id from employee where user_id = (select id from users where email = "igalaviz@allabordo.com")) and cash_out_status = 1)
, (SELECT @idPaymentGarantia ), 2096.0 , '0' , ( select id from users where email = "igalaviz@allabordo.com" ) );
...
values ( (select id from rental where reservation_code = "R21023288B6") , 10 , 6 , 0 , 0 , 6 , 0 , 0.0 , 0.0 , 1 , 1 , ( select id from users where email = "igalaviz@allabordo.com" ) );
