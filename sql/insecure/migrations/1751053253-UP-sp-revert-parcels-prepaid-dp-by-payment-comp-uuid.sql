-- 1751053253 UP sp-revert-parcels-prepaid-dp-by-payment-comp-uuid
DROP PROCEDURE IF EXISTS `revert_parcels_prepaid_debt_payments_by_payment_complement_uuid`;
DELIMITER $$

CREATE DEFINER = CURRENT_USER PROCEDURE `revert_parcels_prepaid_debt_payments_by_payment_complement_uuid`(IN complement_uuid_input VARCHAR(50))
BEGIN
  ...
  pc_loop: LOOP
    ...
    BEGIN
      ...
      dp_loop: LOOP
        FETCH dp_cursor
          INTO v_debt_payment_id, v_parcel_prepaid_id, v_customer_id, v_amount, v_payment_complement_id_inner;
        IF done_debts THEN
          LEAVE dp_loop;
        END IF;

        -- 1) Deuda original del envío prepaid
        SELECT debt
        ...
        -- 2) Revertir deuda en parcels_prepaid
        ...
        -- 4) Deuda final del envío prepaid
        SELECT debt
          INTO v_debt_final
          FROM parcels_prepaid
         WHERE id = v_parcel_prepaid_id;
...
GRANT EXECUTE ON PROCEDURE revert_parcels_prepaid_debt_payments_by_payment_complement_uuid TO 'abordo'@'%';