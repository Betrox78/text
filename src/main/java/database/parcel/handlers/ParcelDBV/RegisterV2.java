...
public class RegisterV2 extends DBHandler<ParcelDBV> {

    public RegisterV2(ParcelDBV dbVerticle) {
        super(dbVerticle);
    }
        ...
        try {
            ...
            CompletableFuture.allOf(cityValidations.toArray(new CompletableFuture[cityValidations.size()])).whenComplete((rCityValidations, errCityValidations) -> {
                try {
                    ...
                    createParcel(conn, parcel, cashRegisterId, isInternalParcel, integrationPartnerSessionId, purchaseOrigin).whenComplete((parcelId, errorCreateParcel) -> {
                        try {
                                        ...
                                        try {
                                            ...
                                            for (int i = 0; i < discountParcelPackages.size(); i++) {
                                                ...
                                                for (int j = 0; j < parcelPackageGrouped.getInteger(_QUANTITY); j++) {
                                                    packagesTasks.add(registerPackagesInParcel(conn, parcelPackageGrouped, parcelId, finalPaysSender, createdBy, terminalOriginId, withoutFreight, isInternalParcel, isPendingCollection));
                                                }
                                            }
                                                    ...
                                                    CompletableFuture.allOf(parcelPackings.stream()
                                                            ...
                                                            .whenComplete((ks, kt) -> {
                                                                try {
                                                                    ...
                                                                    JsonObject parcelTotalAmount = this.getParcelTotalAmountExec(discountService, parcelPackings, reissue, withoutFreight, cumulativeCost);
                                                                    ...
                                                                    Double parcelIva = discountService.getDouble(_PARCEL_IVA, 0.0);
                                                                    Double totalAmount = parcelTotalAmount.getDouble(_TOTAL_AMOUNT, 0.0);
                                                                    Double excessAmount = discountService.getDouble(_EXCESS_AMOUNT, 0.0);
                                                                    Double excessDiscount = discountService.getDouble(_EXCESS_DISCOUNT, 0.0);
                                                                    ...
                                                                    insuranceValidations(insuranceValue, ivaPercent, invoiceValue).whenComplete((insuranceObj, errInsurance) -> {
                                                                        try {
                                                                            ...
                                                                            this.endRegister(conn, cashOutId, parcel, parcelId, createdBy, payments, isComplement,
                                                                                        cashChange, currencyId, discountParcelPackages, parcelPackings, objPayback,
                                                                                        paybackPoints, paybackMoney, isInternalParcel, isCredit, customerCreditData, reissue , shipmentType, typeServiceId, zipCodeService, purchaseOrigin).whenComplete((resultRegister, errorRegister) -> {
                                                                                try {
                                                                                    if (errorRegister != null){
                                                                                        throw errorRegister;
                                                                                    }
    ...
    private CompletableFuture<JsonObject> insuranceValidations(Double insuranceValue, Double ivaPercent, Double invoiceValue) throws ParseException {
        CompletableFuture<JsonObject> future = new CompletableFuture<>();
        ...
        if (Objects.nonNull(insuranceValue) && insuranceValue > 0) {
            ...
            String configInsurancePercent = Objects.isNull(invoiceValue) ? "insurance_percent" : "insurance_percent_inhouse";
            this.getVertx().eventBus().send(GeneralConfigDBV.class.getSimpleName(),
                    new JsonObject().put("fieldName", configInsurancePercent),
    ...
    private JsonObject getParcelTotalAmountExec(JsonObject parcel, JsonArray parcelPackings, Boolean reissue, Boolean withoutFreight, Boolean cumulativeCost){
        JsonObject withoutFreightReturn = new JsonObject()
                .put(_AMOUNT, 0.00)
                .put(_DISCOUNT, 0.00)
                .put(_TOTAL_AMOUNT, 0.00)
                .put(_EXTRA_CHARGES, 0.00)
                .put(_IVA, 0.00);
        if (!reissue){
            return this.getParcelTotalAmount(parcel, parcelPackings);
        } else {
    ...
    private CompletableFuture<Boolean> registerPackagesInParcel(SQLConnection conn, JsonObject parcelPackage, Integer parcelId, Boolean paysSender, Integer createdBy, Integer terminalId, Boolean withoutFreight, Boolean isInternalParcel, boolean isPendingCollection) {
        CompletableFuture<Boolean> future = new CompletableFuture<>();
        try {
            ...
            if (withoutFreight){
                parcelPackage.put(_AMOUNT, 0.00);
                parcelPackage.put(_DISCOUNT, 0.0);
                parcelPackage.put(_TOTAL_AMOUNT, 0.00);
            } else {
...
//        });
    ...
    protected CompletableFuture<JsonObject> endRegister(SQLConnection conn, Integer cashOutId, JsonObject parcel, Integer parcelId, Integer createdBy,
                                                        ...
                                                        JsonArray parcelPackings, PaybackDBV objPayback, Double paybackPoints, Double paybackMoney,  Boolean isInternalParcel, Boolean is_credit,
        ...
        try {
            ...
            if (!paysSender) {
                ...
                if(payments == null && !is_complement && !isInternalParcel){
    ...
    private CompletableFuture<Boolean> insertTicketDetail(SQLConnection conn, Integer ticketId, Integer createdBy, JsonArray packages, JsonArray packings, JsonObject parcel, Boolean isInternalParcel, SHIPMENT_TYPE shipmentType, Double servicesAmount) {
        CompletableFuture<Boolean> future = new CompletableFuture<>();
        try{
            Map<String, List<JsonObject>> groupedPackages = packages.stream().map(x -> (JsonObject)x).collect(Collectors.groupingBy(w -> w.getString(PACKAGE_PRICE_NAME)));
